...
class LoRATrainer:
    ...
    def _train_with_fabric(
        ...
        # Optimize Tensor Core usage on Ampere+ GPUs
        ...
        for epoch in range(start_epoch, self.training_config.max_epochs):
            ...
            if (epoch + 1) % self.training_config.save_every_n_epochs == 0:
                checkpoint_dir = os.path.abspath(os.path.join(self.training_config.output_dir, "checkpoints", f"epoch_{epoch+1}"))
                save_training_checkpoint(
                    self.module.model,
                    optimizer,
                    scheduler,
        ...
        # Save final model (use absolute path)
        final_path = os.path.abspath(os.path.join(self.training_config.output_dir, "final"))
        save_lora_weights(
            self.module.model, final_path,
            trigger_tag=self.training_config.trigger_tag,
            tag_position=self.training_config.tag_position,
        )
    ...
    def _train_basic(
        ...
        for epoch in range(self.training_config.max_epochs):
            ...
            if (epoch + 1) % self.training_config.save_every_n_epochs == 0:
                ...
                save_lora_weights(
                    self.module.model, checkpoint_dir,
                    trigger_tag=self.training_config.trigger_tag,
                    tag_position=self.training_config.tag_position,
                )

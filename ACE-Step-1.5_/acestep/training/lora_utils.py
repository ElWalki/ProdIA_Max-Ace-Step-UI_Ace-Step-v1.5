...
def save_lora_weights(
    ...
    Args:
        ...
        trigger_tag: Custom activation tag used during training
        tag_position: Position of the tag ("prepend" or "append")
        description: Description of the LoRA adapter
        
    Returns:
        Path to saved weights
    """
    # Convert to absolute path to avoid CWD issues with Fabric workers
    output_dir = os.path.abspath(output_dir)
    os.makedirs(output_dir, exist_ok=True)
    
    # Unwrap Fabric _FabricModule if present
    decoder = getattr(model, 'decoder', None)
    if decoder is not None:
        # Fabric wraps modules - unwrap to get the real PeftModel
        if hasattr(decoder, '_forward_module'):
            decoder = decoder._forward_module
        elif hasattr(decoder, 'module'):
            decoder = decoder.module
    
    if decoder is not None and hasattr(decoder, 'save_pretrained'):
        # Save PEFT adapter
        adapter_path = os.path.join(output_dir, "adapter")
        decoder.save_pretrained(adapter_path)
        
        # Save lora_metadata.json alongside the adapter
        _save_lora_metadata(adapter_path, trigger_tag, tag_position, description)
...
def _save_lora_metadata(
    adapter_path: str,
    trigger_tag: str = "",
    tag_position: str = "prepend",
    description: str = "",
):
    """Save lora_metadata.json alongside a LoRA adapter.
    
    This file stores custom metadata not included in adapter_config.json,
    such as the trigger/activation tag used during training.
    
    Args:
        adapter_path: Path to the adapter directory
        trigger_tag: Custom activation tag (e.g., "Walki-bass")
        tag_position: Where to place the tag ("prepend" or "append")
        description: Human-readable description of the adapter
    """
    import json
    
    metadata = {
        "trigger_tag": trigger_tag or "",
        "tag_position": tag_position or "prepend",
    ...
    meta_path = os.path.join(adapter_path, "lora_metadata.json")
    try:
        with open(meta_path, 'w', encoding='utf-8') as f:
            json.dump(metadata, f, indent=2, ensure_ascii=False)
        logger.info(f"LoRA metadata saved to {meta_path}")
    except Exception as e:
        logger.warning(f"Could not save LoRA metadata: {e}")

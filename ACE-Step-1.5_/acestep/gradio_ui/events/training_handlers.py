...
def scan_directory(
    ...
    if not audio_dir or not audio_dir.strip():
        return [], "❌ Please enter a directory path", gr.Slider(minimum=0, maximum=1, value=0), builder_state
    ...
    if not samples:
        return [], status, gr.Slider(minimum=0, maximum=1, value=0), builder
...
def load_existing_dataset_for_preprocess(
    ...
    if not dataset_path or not dataset_path.strip():
        return ("❌ Please enter a dataset path", [], gr.Slider(minimum=0, maximum=1, value=0), builder_state) + empty_preview

    dataset_path = dataset_path.strip()

    if not os.path.exists(dataset_path):
        return (f"❌ Dataset not found: {dataset_path}", [], gr.Slider(minimum=0, maximum=1, value=0), builder_state) + empty_preview
    ...
    if not samples:
        return (status, [], gr.Slider(minimum=0, maximum=1, value=0), builder) + empty_preview
...
def _restore_offloaded_components(dit_handler, llm_handler, offloaded_components):
    """Restore offloaded components back to GPU after training."""
    import torch
    
    if not offloaded_components:
        return
    
    device = getattr(dit_handler, 'device', 'cuda')
    if device == 'cpu':
        device = 'cuda'
    ...
    # Restore LM
    if llm_handler is not None and "LM (PyTorch)" in offloaded_components:
        try:
            if getattr(llm_handler, 'llm', None) is not None and hasattr(llm_handler.llm, 'to'):
                llm_handler.llm.to(device)
                restored.append("LM")
        except Exception as e:
            logger.warning(f"Could not restore LM to GPU: {e}")
    
    # Restore VAE
    if "VAE" in offloaded_components:
        try:
            if getattr(dit_handler, 'vae', None) is not None and hasattr(dit_handler.vae, 'to'):
                dit_handler.vae.to(device)
                restored.append("VAE")
        except Exception as e:
            logger.warning(f"Could not restore VAE to GPU: {e}")
    
    # Restore Text Encoder
    if "Text Encoder" in offloaded_components:
        try:
            if getattr(dit_handler, 'text_encoder', None) is not None and hasattr(dit_handler.text_encoder, 'to'):
                dit_handler.text_encoder.to(device)
                restored.append("Text Encoder")
        except Exception as e:
            logger.warning(f"Could not restore text encoder to GPU: {e}")
    
    if restored:
        logger.info(f"Restored to GPU after training: {', '.join(restored)}")
...
def start_training(
    ...
    # ===== FREE GPU MEMORY: Offload LM, VAE and text_encoder to CPU =====
    import torch
    import gc
    offloaded_components = []
    
    # Log VRAM before offloading
    if torch.cuda.is_available():
        vram_before = torch.cuda.memory_allocated() / 1024**3
        vram_reserved_before = torch.cuda.memory_reserved() / 1024**3
        logger.info(f"VRAM before offload: {vram_before:.2f} GB allocated, {vram_reserved_before:.2f} GB reserved")
    
    # Offload LM (1.7B+ params, not needed for LoRA training)
    if llm_handler is not None and getattr(llm_handler, 'llm', None) is not None:
        try:
            if llm_handler.llm_backend == "pt" and hasattr(llm_handler.llm, 'to'):
                llm_handler.llm.to("cpu")
                offloaded_components.append("LM (PyTorch)")
            elif llm_handler.llm_backend == "vllm":
                # For vLLM/nano-vllm: must delete all references and force GC
                # vLLM pre-allocates CUDA memory that won't free with just del
                llm_ref = llm_handler.llm
                llm_handler.llm = None
                llm_handler.llm_initialized = False
                # Clear any internal caches
                if hasattr(llm_ref, 'llm_engine'):
                    if hasattr(llm_ref.llm_engine, 'model_executor'):
                        del llm_ref.llm_engine.model_executor
                    del llm_ref.llm_engine
                del llm_ref
                offloaded_components.append("LM (vLLM, deleted)")
            # Force garbage collection to release CUDA tensors
            gc.collect()
            torch.cuda.empty_cache()
        except Exception as e:
            logger.warning(f"Could not offload LM: {e}")
    
    # Offload VAE (not needed for LoRA training on preprocessed tensors)
    if getattr(dit_handler, 'vae', None) is not None and hasattr(dit_handler.vae, 'to'):
        try:
            dit_handler.vae.to("cpu")
            offloaded_components.append("VAE")
        except Exception as e:
            logger.warning(f"Could not offload VAE: {e}")
    
    # Offload text encoder (not needed for LoRA training on preprocessed tensors) 
    if getattr(dit_handler, 'text_encoder', None) is not None and hasattr(dit_handler.text_encoder, 'to'):
        try:
            dit_handler.text_encoder.to("cpu")
            offloaded_components.append("Text Encoder")
        except Exception as e:
            logger.warning(f"Could not offload text encoder: {e}")
    
    # Offload the encoder part of the DiT model (only decoder is needed for LoRA)
    if getattr(dit_handler, 'model', None) is not None and hasattr(dit_handler.model, 'encoder'):
        try:
            dit_handler.model.encoder.to("cpu")
            offloaded_components.append("DiT Encoder")
        except Exception as e:
            logger.warning(f"Could not offload DiT encoder: {e}")
    ...
    if offloaded_components:
        logger.info(f"Offloaded to CPU for training: {', '.join(offloaded_components)}")
    
    if torch.cuda.is_available():
        vram_after = torch.cuda.memory_allocated() / 1024**3
        vram_reserved_after = torch.cuda.memory_reserved() / 1024**3
        logger.info(f"VRAM after offload: {vram_after:.2f} GB allocated, {vram_reserved_after:.2f} GB reserved")
        logger.info(f"VRAM freed: {vram_before - vram_after:.2f} GB")
    
    try:
        from acestep.training.trainer import LoRATrainer
        from acestep.training.configs import LoRAConfig as LoRAConfigClass, TrainingConfig
        
        # Read trigger_tag from manifest.json in tensor_dir
        _trigger_tag = ""
        _tag_position = "prepend"
        manifest_path = os.path.join(tensor_dir, "manifest.json")
        if os.path.exists(manifest_path):
            try:
                with open(manifest_path, 'r') as f:
                    manifest = json.load(f)
                metadata = manifest.get("metadata", {})
                _trigger_tag = metadata.get("custom_tag", "")
                _tag_position = metadata.get("tag_position", "prepend")
                if _trigger_tag:
                    logger.info(f"Found trigger_tag in manifest: '{_trigger_tag}' (position: {_tag_position})")
            except Exception as e:
                logger.warning(f"Failed to read trigger_tag from manifest: {e}")
        ...
        # Restore offloaded components back to GPU
        _restore_offloaded_components(dit_handler, llm_handler, offloaded_components)
        ...
        # Restore offloaded components even on error
        _restore_offloaded_components(dit_handler, llm_handler, offloaded_components)
...
def export_lora(
    ...
    # Check if there's a trained model to export (use absolute paths)
    lora_output_dir = os.path.abspath(lora_output_dir)

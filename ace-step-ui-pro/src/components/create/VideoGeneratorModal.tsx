...
export const VideoGeneratorModal: React.FC<VideoGeneratorModalProps> = ({ isOpen, onClose, song }) => {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/90 backdrop-blur-md p-0 md:p-4 animate-in fade-in duration-200">
      ...
      <div className={`bg-[#0c0c12] w-full h-full md:max-w-7xl md:h-[90vh] md:rounded-2xl border-0 md:border border-white/10 overflow-hidden shadow-2xl relative ${isMobile ? 'flex flex-col' : 'flex'}`}>

        {/* Close Button */}
        <button onClick={onClose} className="absolute top-3 right-3 md:top-4 md:right-4 z-50 p-2 bg-black/60 hover:bg-red-500/80 rounded-full text-white/70 hover:text-white transition-all duration-200 active:scale-90 backdrop-blur-sm">
          ...
          <div className="relative bg-black flex-shrink-0">
            ...
            <div className="absolute top-0 left-0 right-0 z-10 p-3 bg-gradient-to-b from-black/80 to-transparent">
              <h2 className="text-base font-bold text-white flex items-center gap-2">
                <Video className="text-accent-400" size={18} />
                Video Studio
              </h2>
            </div>
        ...
        {/* Sidebar Controls */}
        <div className={`${isMobile ? 'flex-1 overflow-hidden' : 'w-96'} bg-[#0f0f15] ${isMobile ? '' : 'border-r border-white/5'} flex flex-col z-20`}>
            {/* Header - Desktop only */}
            {!isMobile && (
              <div className="p-6 border-b border-white/5">
                  <h2 className="text-xl font-bold text-white mb-1 flex items-center gap-2">
                      <Video className="text-accent-400" size={20} />
                      Video Studio
                  </h2>
                  <p className="text-surface-500 text-xs">Create professional visualizers for your music.</p>
              </div>
            )}
            ...
            <div className="flex border-b border-white/5">
                {[
                    ...
                    <button 
                        ...
                        className={`flex-1 flex items-center justify-center gap-2 py-3 text-xs font-bold uppercase tracking-wider transition-colors ${activeTab === tab.id ? 'text-white border-b-2 border-accent-400 bg-white/5' : 'text-zinc-500 hover:text-zinc-300'}`}
                    >
                        {tab.icon} {tab.label}
                    </button>
                ))}
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        {PRESETS.map(preset => (
                            <button
                                ...
                                className={`flex flex-col items-center gap-2 p-4 rounded-xl border transition-all ${config.preset === preset.id ? 'bg-accent-500/15 border-accent-400 text-white' : 'bg-black/20 border-white/5 text-zinc-400 hover:bg-white/5 hover:border-white/10'}`}
                            >
                            ...
                            <div className="bg-black/20 p-3 rounded-lg border border-white/5 space-y-3">
                                ...
                                <div className="grid grid-cols-3 gap-2">
                                     <button
                                        ...
                                        className={`py-2 rounded text-xs font-bold flex items-center justify-center gap-1 ${backgroundType === 'random' ? 'bg-accent-500 text-white' : 'bg-zinc-800 text-zinc-400'}`}
                                     >
                                         <Wand2 size={12}/> Random
                                     </button>
                                     <button
                                        onClick={() => setBackgroundType('custom')}
                                        className={`py-2 rounded text-xs font-bold flex items-center justify-center gap-1 ${backgroundType === 'custom' ? 'bg-accent-500 text-white' : 'bg-zinc-800 text-zinc-400'}`}
                                     >
                                         <ImageIcon size={12}/> Image
                                     </button>
                                     <button
                                        onClick={() => setBackgroundType('video')}
                                        className={`py-2 rounded text-xs font-bold flex items-center justify-center gap-1 ${backgroundType === 'video' ? 'bg-accent-500 text-white' : 'bg-zinc-800 text-zinc-400'}`}
                                     >
                                         <Video size={12}/> Video
                                     </button>
                                </div>
                ...
                {activeTab === 'text' && (
                    <div className="space-y-4">
                        <button 
                            ...
                            className="w-full py-2 bg-accent-500 text-white rounded-lg flex items-center justify-center gap-2 text-xs font-bold hover:bg-accent-600"
                        >
                    ...
                    <div className="space-y-2">
                        {[
                            ...
                            { id: 'letterbox', label: 'Letterbox', desc: 'Cinematic bars', icon: <Minus size={16}/> },
                             ...
                             return (
                                <div key={effect.id} className={`rounded-lg border transition-all ${isActive ? 'bg-accent-500/10 border-accent-500/25' : 'bg-black/20 border-white/5'}`}>
                                     <button 
                                    ...
                                    >
                                        ...
                                        <div className={`w-3 h-3 rounded-full ${isActive ? 'bg-accent-500 shadow-[0_0_8px_color-mix(in_srgb,var(--color-accent-500)_80%,transparent)]' : 'bg-zinc-700'}`}></div>
                                    </button>
            ...
            <div className="p-4 md:p-6 border-t border-white/5 bg-black/20 space-y-3 safe-area-inset-bottom">
                 {ffmpegLoading ? (
                     ...
                     <div className="w-full bg-zinc-800 rounded-xl h-12 flex items-center justify-center px-4 relative overflow-hidden">
                         <div
                           className={`absolute left-0 top-0 bottom-0 transition-all duration-100 ${exportStage === 'capturing' ? 'bg-accent-500/15' : 'bg-blue-600/20'}`}

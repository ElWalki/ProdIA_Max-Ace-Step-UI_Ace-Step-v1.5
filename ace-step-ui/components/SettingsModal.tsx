...
export const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose, theme, onToggleTheme, onNavigateToProfile }) => {
    ...
    const handleScanVram = async () => {
        ...
        try {
            const result = await vramApi.status(token);
    ...
    const handleForceCleanup = async () => {
        ...
        try {
            ...
            const freedMb = result?.nvidia_freed_mb ?? 0;
            setCleanupResult(`✅ Purge done — freed ${freedMb.toFixed(0)} MB VRAM`);
            // Auto-rescan after cleanup
            setTimeout(() => handleScanVram(), 1000);
        } catch (err) {
            setCleanupResult('❌ VRAM purge failed');
        } finally {
